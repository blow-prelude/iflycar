...
4. 上下翼判向:箭头的上、下两翼只存在于指向侧,箭杆穿过水平中部。逐行统计连通主体像素,取最宽行(杆行)为基准,按 `杆行 ± 高度/4` 划分上翼带/杆带/下翼带并求各带列均值;要求上翼与下翼相对杆带的水平偏移都 ≥ 5%×主体宽度且**同号**(同在杆的一侧):两翼同在杆右侧判 `right`,同在左侧判 `left`;偏移不足或两翼异号判 `unknown`。双翼同侧 + 双侧最小偏移两个约束同时成立才输出方向,单个亮斑或箭杆断裂很难同时满足,比旧的单值偏度符号更抗噪。
