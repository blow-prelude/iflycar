...
from camera_2d_lidar_calibration.msg import vision_points
...
def callback(scan, image):
    ...
    # cv2.waitKey(1) #等待按键
    # print("ok")
    msg_points_x = [-255] * 641
    msg_points_y = [-255] * 641
    msg_points_z = [-255] * 641
    for i in range(len(img_points)):
        if( int(round(img_points[i][0]))<0 or int(round(img_points[i][0]))>640 ):
            continue
        msg_points_x[int(round(img_points[i][0]))] = objPoints[0][i][0]
        msg_points_y[int(round(img_points[i][0]))] = objPoints[0][i][1]
        msg_points_z[int(round(img_points[i][0]))] = objPoints[0][i][2]
        # print("x_px:{},({},{},{})".format(int(round(img_points[i][0])),objPoints[0][i][0],objPoints[0][i][1],objPoints[0][i][2]))
    msg_pub = vision_points()
    msg_pub.dx = msg_points_x
    msg_pub.dy = msg_points_y
    msg_pub.dz = msg_points_z
    # print(msg_points_y)
    pub_vision.publish(msg_pub)
...
pub_vision = rospy.Publisher("vision_points",vision_points,queue_size=1)
