...
private:
    // 配置参数
    const double MAX_ANGULAR_VEL = 0.5;       // 最大角速度 (rad/s)
    ...
    const int Y_LOWER_BOUND = 270;            // y 值有效范围下限
    const int Y_UPPER_BOUND = 300;            // y 值有效范围上限
    ...
    double calculateLinearPID(double err) {
        ...
        // 积分限幅
        if (fabs(linear_pid_.err) < 0.1) {  // 误差较小时累加积分
