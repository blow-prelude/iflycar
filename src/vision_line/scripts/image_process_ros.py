...
class ImageProcessConfig:
    ...
    search_range_wide: int = 50  # y_norm > threshold 时的搜索范围
    search_range_narrow: int = 30  # y_norm <= threshold 时的搜索范围
    search_range_threshold: float = 0.6
    min_left_right_distance: int = 30  # 左右边线最小间距阈值，过小则认为是噪点
    ...
    # ---- 多项式拟合（分段线性） ----
    fit_angle_thresh: float = 30.0
    fit_max_offset: int = 10
    fit_y_div_far_w: float = 0.45
    fit_y_div_near_w: float = 0.55
...
class RuntimeConfig:
    ...
    corner_delay_s: float = 1.5
    turning_end_x_error_abs_max: float = 15.0
...
class ImageProcess:
    ...
    def perspective_transform(self, img, matrix, output_size=None):
        ...
        rospy.logdebug(f"Perspective transform completed, output size: {output_size}")
    ...
    def fit_polynomial(self):
        ...
        try:
            ...
            if angle > self.cfg.fit_angle_thresh:
                offset = int(
                    -np.sign(k_far) * min(angle / 60.0, 1.0) * self.cfg.fit_max_offset
                )
    ...
    def get_side_line_task_1(self, img, canvas, is_draw=False):
        ...
        try:
            ...
            for y in range(
                ...
                # --- 左侧赛道线 ---
                if left_stable[0]:
                    search_start_left = min(pre_lx + cur_range, img_w - 1)
                    search_end_left = max(0, pre_lx - cur_range)
                else:
                ...
                left_added = False
                if len(candidates) > 0:
                    lx = search_end_left + candidates[-1]
                ...
                if left_miss_count >= self.cfg.miss_threshold:
                    pre_lx = mid_x - self.cfg.search_offset
                    left_miss_count = 0

                # --- 右侧赛道线 ---
                if right_stable[0]:
                    search_start_right = max(0, min(pre_rx - cur_range, img_w - 1))
                    search_end_right = min(img_w - 1, pre_rx + cur_range)
                else:
                ...
                right_added = False
                if len(candidates) > 0:
                    rx = search_start_right + candidates[0]
                ...
                if right_miss_count >= self.cfg.miss_threshold:
                    pre_rx = mid_x + self.cfg.search_offset
                    right_miss_count = 0
        ...
        try:
            ...
            for y in range(
                ...
                if side != SearchSide.RIGHT_ONLY:
                    ...
                    if len(candidates) > 0:
                        lx = candidates[-1] + search_end_left
                ...
                if side != SearchSide.LEFT_ONLY:
                    ...
                    if len(candidates) > 0:
                        rx = candidates[0] + search_start_right
...
def build_vision_line_msg_by_index(
    ...
    if len(line_points) < abs(index) + 1:
        best_x, best_y = int(line_points[-1, 0]), int(line_points[-1, 1])
    ...
    # rospy.loginfo(
    #     f"Selected point  (proc_x={best_x}, proc_y={best_y}) -> (orig_x={x_raw:.2f}, orig_y={y_raw:.2f}), x_error={x_error:.2f}"
    # )
...
def run_ros_topic_mode():
    ...
    imgprocess = ImageProcess(config=ImageProcessConfig())

    img_sender = ImageSender(rt_cfg.img_sender_ip, rt_cfg.img_sender_port)
    ...
    corner_delay_s = rt_cfg.corner_delay_s
