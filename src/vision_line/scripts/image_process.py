...
class ImageProcess:
    ...
    def preprocess(self):
        ...
        try:
            ...
            # 做膨胀操作
            kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3))
            dilate = cv2.dilate(binary, kernel, iterations=2)  # 用膨胀操作连接边线
            return dilate
            ...
            for x in x_range:
                ...
                if prev_cluster_rep is not None:
                    ...
                    if dy > 0:
                        ...
                        if (
                            prev_abs_slope is not None
                            and prev_abs_slope < small
                            and curr_abs_slope > large
                        ):
    ...
    def get_side_line_task_1(self, img, canvas, is_draw=False, turn_right=True):
        ...
        try:
            ...
            corner_y_thresh = int(img.shape[0] * 0.75)
            allow_horiz_search = (
                left_corner is not None
                and right_corner is not None
                and left_corner[1] >= corner_y_thresh
                and right_corner[1] >= corner_y_thresh
            )

            horiz_corner = None
            exclude_center = None

            if allow_horiz_search:
                HORIZ_UP_RATIO = 0.50
                HORIZ_DOWN_RATIO = 0.75
            ...
            logging.info(
                "[horiz] allow=%s turn_right=%s corner=%s exclude_center=%s",
                allow_horiz_search,
                turn_right,
                horiz_corner,
                exclude_center,
