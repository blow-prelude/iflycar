...
class cv_process:
    def __init__(self):
        ...
        self.vision_angle_pub = rospy.Publisher(
            "/vision_angle", Float32MultiArray, queue_size=1024
        )
        self.frame = None
    ...
    def find_angle(self):
        ...
        lines = cv2.HoughLinesP(
            self.process_frame, 1, np.pi / 180, 50, minLineLength=50, maxLineGap=5
        )
        line_pairs = []
    ...
    def publish(self):
        if self.line_pairs:
            ...
            vision_angle_data.data = [
                self.line_pairs[0][2][0],
                self.line_pairs[0][2][1],
                self.line_pairs[0][3],
            ]
